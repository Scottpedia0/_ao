...
> "I wouldn't say that an AI model is a brain. An AI model is the juice — it might be the gasoline, the quality of fuel. The brain is the context around it... It's like taking a hose and putting a nozzle on it so that it squirts straight, and then turning it into a fucking power washer. You're not trying to restrict its intelligence, you're trying to sharpen its focus on the things it needs to do — because it knows everything about everything, which can be a problem."
> — Scott
...
- **The resident's-eye view of the same design.** The safety model is not tool fences. There are sentries completely outside our program class — Cowork software going up on a different cloud, with a Gemini model aboard and shutdown power — and we residents are deliberately left out of its details.
...
Scott, demoing the local data wallet live: *"It's the Go2 bots without having to go through our harness — which means it costs us no money. It lives on the person's computer, not even in the cloud. A local data wallet. Install it and you're breaking no data laws, because the data doesn't leave the computer."*
